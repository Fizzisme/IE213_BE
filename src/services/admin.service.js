...
// ============================================================================
// METAMASK FLOW: Admin operations with prepare/confirm pattern
// ============================================================================

/**
 * Step 1: PREPARE ADD DOCTOR TX
 * GET /admin/prepare-add-doctor
 *
 * Admin prepares unsigned transaction to add doctor role
 */
const prepareAddDoctorTx = async (adminAddress, walletAddressToAdd) => {
    const txData = await metaMaskTxBuilder.prepareAddDoctorTx(adminAddress, walletAddressToAdd)
    return {
        success: true,
        data: txData
    }
...
/**
 * Step 2: CONFIRM ADD DOCTOR TX
 * POST /admin/confirm-add-doctor
 *
 * Admin broadcasts signed transaction and backend updates DB
 */
const confirmAddDoctorTx = async (signedTx, adminAddress, doctorWalletAddress, doctorUserData) => {
    // Verify and broadcast signed transaction first
    const txVerification = await metaMaskTxBuilder.verifyAndBroadcastSignedTx(
        signedTx,
        adminAddress,
        'confirmAddDoctorTx'
    )

    // Now create doctor account off-chain with blockchain tx hash
    let result
    try {
        result = await createDoctor({
            email: doctorUserData.email,
            password: doctorUserData.password,
            nationId: doctorUserData.nationId,
            walletAddress: doctorWalletAddress,
            adminId: null
        })
    } catch (createError) {
        // Nếu lỗi CONFLICT (email đã tồn tại), chỉ ghi audit log
        if (createError.status === StatusCodes.CONFLICT) {
            console.log(`[WARN] Doctor account already exists, only blockchain tx recorded: ${doctorWalletAddress}`)
            const existingUser = await userModel.UserModel.findOne({
                'authProviders.email': doctorUserData.email,
                _destroy: false
            })
            result = {
    ...
    await auditLogModel.createLog({
        ...
        status: 'SUCCESS',
        details: {
            note: `Admin added doctor role via MetaMask: ${doctorWalletAddress}`,
            doctorWalletAddress,
            txHash: txVerification.txHash
        }
    })
    ...
    return {
        success: true,
        message: 'Doctor account created and blockchain tx confirmed',
        data: {
...
/**
 * Step 1: PREPARE ADD LAB TECH TX
 * GET /admin/prepare-add-labtech
 */
const prepareAddLabTechTx = async (adminAddress, walletAddressToAdd) => {
    const txData = await metaMaskTxBuilder.prepareAddLabTechTx(adminAddress, walletAddressToAdd)
    return {
        success: true,
        data: txData
    }
...
/**
 * Step 2: CONFIRM ADD LAB TECH TX
 * POST /admin/confirm-add-labtech
 */
const confirmAddLabTechTx = async (signedTx, adminAddress, labTechWalletAddress, labTechUserData) => {
    // Verify and broadcast signed transaction first
    const txVerification = await metaMaskTxBuilder.verifyAndBroadcastSignedTx(
        signedTx,
        adminAddress,
    ...
    // Now create lab tech account off-chain with blockchain tx hash
    let result
    try {
        result = await createLabTech({
            email: labTechUserData.email,
            password: labTechUserData.password,
            nationId: labTechUserData.nationId,
        ...
        // Nếu lỗi CONFLICT (email đã tồn tại), chỉ ghi audit log
        if (createError.status === StatusCodes.CONFLICT) {
            console.log(`[WARN] Lab tech account already exists, only blockchain tx recorded: ${labTechWalletAddress}`)
            const existingUser = await userModel.UserModel.findOne({
                'authProviders.email': labTechUserData.email,
                _destroy: false
            })
            result = {
                userId: existingUser._id,
    ...
    await auditLogModel.createLog({
        ...
        status: 'SUCCESS',
        details: {
            note: `Admin added lab tech role via MetaMask: ${labTechWalletAddress}`,
    ...
    return {
        success: true,
        message: 'Lab tech account created and blockchain tx confirmed',
        data: {
            userId: result.userId,
            labTechWalletAddress,
            txHash: txVerification.txHash
...
/**
 * Step 1: PREPARE REGISTER PATIENT TX
 * GET /admin/prepare-register-patient
 */
const prepareRegisterPatientTx = async (adminAddress, patientWalletAddress) => {
    const txData = await metaMaskTxBuilder.prepareRegisterPatientTx(adminAddress, patientWalletAddress)
    return {
        success: true,
        data: txData
    }
...
/**
 * Step 2: CONFIRM REGISTER PATIENT TX
 * POST /admin/confirm-register-patient
 */
const confirmRegisterPatientTx = async (signedTx, adminAddress, patientWalletAddress, patientUserId) => {
    // Verify wallet address format
    if (!ethers.isAddress(patientWalletAddress)) {
        throw new ApiError(StatusCodes.BAD_REQUEST, 'Invalid patient wallet address format')
    }

    // Verify and broadcast signed transaction
    const txVerification = await metaMaskTxBuilder.verifyAndBroadcastSignedTx(
        signedTx,
        adminAddress,
        'confirmRegisterPatientTx'
    )
    ...
    await auditLogModel.createLog({
        ...
        action: 'ADMIN_REGISTER_PATIENT_BLOCKCHAIN',
        entityType: 'USER',
        entityId: patientUserId,
        txHash: txVerification.txHash,
        status: 'SUCCESS',
        details: {
            note: `Admin registered patient via MetaMask: ${patientWalletAddress}`,
    ...
    return {
        success: true,
        message: 'Patient registered on blockchain via admin MetaMask',
        data: {
            patientUserId,
            patientWalletAddress,
            txHash: txVerification.txHash
...
export const adminService = {
    ...
    // MetaMask prepare/confirm operations
    prepareAddDoctorTx,
    confirmAddDoctorTx,
    prepareAddLabTechTx,
    confirmAddLabTechTx,
