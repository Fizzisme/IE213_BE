...
## 12) Lo trinh trien khai nhanh (7 ngay)

### Ngay 1-2
- Tach state machine on-chain status.
- Them endpoint create + onchain-submit.

### Ngay 3
- Them worker confirm tx + retry/backoff.

### Ngay 4
- Cache hasAccess (TTL ngan) + invalidation.

### Ngay 5
- Hoan thien canonical hash + endpoint verify.

### Ngay 6
- Them metrics va log cho slow query / failed tx.

### Ngay 7
- Chay smoke test va load test nhe, chot KPI demo.

---

## 13) Checklist demo bao cao

- [ ] Tao record tra ket qua ngay (< 1s UX)
- [ ] UI hien badge `PENDING -> CONFIRMED`
- [ ] Verify hash thanh cong voi du lieu nguyen ven
- [ ] Sua du lieu Mongo gia lap -> verify fail
- [ ] Log/audit day du cho create, confirm, verify
